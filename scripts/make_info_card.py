...
# ---- config: who this profile belongs to -----------------------------------
HOST = "lakshanyakrishna"
...
ROWS = [
    ("host",),
    ("kv", "ROLE", "Software Development Intern @ Fraylon"),
    ("kv", "EDUCATION", "B.Tech AI & Data Science"),
    ("kv", "STACK", "React · Node · Express · MongoDB"),
    ("kv", "LANGUAGES", "JavaScript · Python"),
    ("kv", "DATABASES", "MongoDB · PostgreSQL · MySQL"),
    ("kv", "UI/UX", "Figma · Responsive Design"),
    ("kv", "AI", "Prompt Engineering"),
    ("gap",),
    ("sec", "Projects"),
    ("bul", "Brand Forge"),
    ("bul", "Learning Management System"),
    ("bul", "Hackathon Management Platform"),
    ("gap",),
    ("kv", "INTEREST", "AI Design Systems"),
    ("kv", "LOCATION", "Hyderabad, India"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">{HOST}@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 ...
                 f'<line x1="{KEY_X + 8 + len(HOST + "@github")*8.4:.0f}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
